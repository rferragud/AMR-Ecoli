...
# 7. Transformación de mic a sir ####

## 7.1. EUCAST ####

broth_microdil_sir_EUCAST <- broth_microdil %>%
  filter(!internal_study_name == "rebelo2022") %>% #eliminamos este estudio por tener datos mic poco estandares
  ...
  # 5) agrupar por antibiótico y MO, y llamar as.sir() **una sola vez** por grupo
  group_by(ab, mo) %>%
  mutate(
    EUCAST_2025 = as.sir(
      x         = mic,
...
broth_microdil_sir_ECOFF <- broth_microdil %>%
...
safe_sir <- function(mic, mo, ab, ecoff, guideline = "EUCAST 2025", breakpoint_type="ECOFF") {
  sapply(mic, function(m) {
    num <- suppressWarnings(as.double(m))
    if ((grepl("^<", m) & num > ecoff) |
        (grepl("^>", m) & num < ecoff) |
        (grepl("^<=", m) & num > ecoff) |
        (grepl("^>=", m) & num < ecoff)) {
      return(NA_sir_)
    }
    as.sir(m, mo = mo, ab = ab, guideline = guideline, breakpoint_type
           = breakpoint_type)
  })
}

broth_microdil_sir_ECOFF.new <- broth_microdil_BP %>%
  filter(!internal_study_name == "rebelo2022") %>% 
  # 1) limpieza básica y unificación de formatos
  mutate(
    meas = measurement_combined %>%
...
broth_microdil_sir_CLSI <- broth_microdil %>%
  filter(!internal_study_name == "rebelo2022") %>% 
  # 1) limpieza básica y unificación de formatos
  mutate(
    meas = measurement_combined %>%
...
historial_pheno_mic <- sir_interpretation_history()
 
historial_pheno_mic %>% count(input) %>% print(n= Inf)
S_R <- historial_pheno_mic %>% filter(ab == "CIP")%>% group_by(outcome) %>% summarise(values = paste(sort(unique(input), decreasing = T), collapse = "; "))

historial_pheno_mic %>% filter(ab == "CIP") %>% pull(input) %>% unique()

write.csv(historial_pheno_mic, "C:/Users/rferragud/Documents/Projecte/Tables/historial_pheno_mic.csv", row.names = FALSE)

# 8. Escribir las tablas ####

EUCAST_sir <- 
  broth_microdil_sir_EUCAST %>% 
  filter(!is.na(EUCAST_2025)) %>%
  select(ENA_experiment_accession:ENA_sample_accession, antibiotic, EUCAST_2025)

ECOFF_sir <- 
  broth_microdil_sir_ECOFF %>% 
  filter(!is.na(ECOFF_2025)) %>%
  select(ENA_experiment_accession:ENA_sample_accession, antibiotic, ECOFF_2025)

CLSI_sir <- 
  broth_microdil_sir_CLSI %>% 
  filter(!is.na(CLSI_2025)) %>%
  select(ENA_experiment_accession:ENA_sample_accession, antibiotic, CLSI_2025)

write.csv(EUCAST_sir, "C:/Users/rferragud/Documents/Projecte/Tables/EUCAST_sir.csv", row.names = FALSE)
write.csv(ECOFF_sir, "C:/Users/rferragud/Documents/Projecte/Tables/ECOFF_sir.csv", row.names = FALSE)
write.csv(CLSI_sir, "C:/Users/rferragud/Documents/Projecte/Tables/CLSI_sir.csv", row.names = FALSE)

EUCAST_sir %>% distinct(antibiotic) %>% print(n =200)
ECOFF_sir %>% distinct(antibiotic) %>% print(n =200)
CLSI_sir %>% distinct(antibiotic) %>% print(n =200)
